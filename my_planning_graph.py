...
class PlanningGraph:
    ...
    def create_graph(self):
        ...
        if (len(self.s_levels) != 0) or (len(self.a_levels) != 0):
            raise Exception(
                'Planning Graph already created; construct a new planning graph for each new state'
    ...
    def add_action_level(self, level):
        ...
        Iterate through all possible actions for the problem and add a PgNode_a to a_levels[0].
        ...
        for action in self.all_actions:
            temp_node_a = PgNode_a(action)
            # determine what actions to add and create those PgNode_a objects
            if temp_node_a.prenodes.issubset(self.s_levels[level]):
                self.a_levels[level].add(temp_node_a)
                for pre_node in temp_node_a.prenodes:
                    for s_node in self.s_levels[level]:
                        if pre_node == s_node:
                            # connect the nodes to the previous S literal level
                            temp_node_a.parents.add(s_node)
    ...
    def add_literal_level(self, level):
        ...
        # each effnode from a node in previous level's A_nodes, becomes a new literal state node
        for a_node in self.a_levels[level-1]:
            node_list = a_node.effnodes
    ...
    def inconsistent_effects_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # compare add effects from node1 to node2 remove effects
        for eff_add in node_a1.action.effect_add:
            if eff_add in node_a2.action.effect_rem:
                return True
        # compare remove effects from node1 to node2 add effects
        for eff_rem in node_a1.action.effect_rem:
            if eff_rem in node_a2.action.effect_add:
                return True
        return False
    ...
    def interference_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # check effects of node1 for conflicts in precond node2
        for add_eff in node_a1.action.effect_add:
            if add_eff in node_a2.action.precond_neg:
                return True
        for rem_eff in node_a1.action.effect_rem:
            if rem_eff in node_a2.action.precond_pos:
                return True
        # check effects of node2 for conflicts in precond node1
        for add_eff in node_a2.action.effect_add:
            if add_eff in node_a1.action.precond_neg:
                return True
        for rem_eff in node_a2.action.effect_rem:
            if rem_eff in node_a1.action.precond_pos:
                return True
        return False
    ...
    def competing_needs_mutex(self, node_a1: PgNode_a, node_a2: PgNode_a) -> bool:
        ...
        :return: bool
        """
        # test for all mutex types between nodes' parents
